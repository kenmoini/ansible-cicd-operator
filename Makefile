...
# kemo.dev/ansible-cicd-operator-bundle:$VERSION and kemo.dev/ansible-cicd-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/kenmoini/cicd-operator
...
bundle-validate: ## Validates the bundle image.
	$(OPERATOR_SDK) bundle validate $(BUNDLE_IMG)

.PHONY: bundle-upgrade
bundle-upgrade: ## Upgrades the bundle image with a new version.
